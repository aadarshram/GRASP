"""
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  Multi-Camera   │────▶│  Vision Tower   │────▶│  MM Projector   │
│  Images (L/R/T) │     │  (CLIP/SigLIP)  │     │  (MLP)          │
└─────────────────┘     └─────────────────┘     └────────┬────────┘
                                                         │
┌─────────────────┐                                      ▼
│  Language       │────▶┌─────────────────┐     ┌─────────────────┐
│  Instruction    │     │  Tokenizer      │────▶│  Pythia LLM     │
└─────────────────┘     └─────────────────┘     └────────┬────────┘
                                                         │
┌─────────────────┐                                      ▼
│  Robot State    │────────────────────────────▶┌─────────────────┐
│  (qpos/qvel)    │                             │  Action Head    │────▶ Action Chunk
└─────────────────┘                             │  (Diffusion)    │      (16 steps)
                                                └─────────────────┘
"""
import pickle
...
sys.path.insert(0, repo_root)  # For src module
sys.path.insert(0, script_dir)  # For aloha_scripts
...
def parse_pythia():
    ...
    It sets the global `local_rank` variable based on the training arguments and configures quantization settings if specified in the training arguments.
...
def main(config=None, llava_pythia_config=None):
    ...
    It loads the training and validation datasets and calls the `train_bc` function to perform the training. After training, it saves dataset statistics.

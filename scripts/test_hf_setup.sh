#!/bin/bash
# filepath: /home/hemanthm/Desktop/GRASP/GRASP/scripts/test_hf_setup.sh
# GRASP VLA Training for HuggingFace metaworld-pick-place-v3 Dataset
# TEST MODE - Quick validation script
...
# ========================
# CONFIGURATION - TEST MODE (all variables set to 1 for quick testing)
# ========================
MODEL_NAME="lesjie/Llava-Pythia-400M"
OUTPUT_DIR="./outputs/metaworld_hf_test"
...
cp "$SCRIPT_DIR/test_hf_setup.sh" "$OUTPUT_DIR/test_hf_setup.sh"
...
$DEEPSPEED_CMD --master_port 29600 --num_gpus=1 --num_nodes=1 \
  ...
  --report_to wandb \
  --run_name "metaworld-hf-test-$(date +%Y%m%d_%H%M%S)" \
  --seed 42
